...
{   a ^= SERPENT(l_key)[4 * r +  8]; \
    b ^= SERPENT(l_key)[4 * r +  9]; \
    c ^= SERPENT(l_key)[4 * r + 10]; \
    d ^= SERPENT(l_key)[4 * r + 11]; \
}
...
{   SERPENT(l_key)[4 * r +  8] = a;  \
    SERPENT(l_key)[4 * r +  9] = b;  \
    SERPENT(l_key)[4 * r + 10] = c;  \
    SERPENT(l_key)[4 * r + 11] = d;  \
}
...
void SERPENT(set_key(const u1byte in_key[], const u4byte key_len, const enum dir_flag fl))
{   u4byte  i,lk,a,b,c,d,e,f,g,h;
    ...
    while(i < lk)
    {
        SERPENT(l_key)[i] = u4byte_in(in_key + 4 * i); i++;
    }
    ...
    if(key_len < 256)
    {
        ...
        SERPENT(l_key)[i] = SERPENT(l_key)[i] & (lk - 1) | lk;
    }

    t1 = SERPENT(l_key)[2] ^ SERPENT(l_key)[4] ^ SERPENT(l_key)[6] ^ 0x9e3779b9;
    t2 = SERPENT(l_key)[3] ^ SERPENT(l_key)[5] ^ SERPENT(l_key)[7] ^ 0x9e3779b9;

    for(i = 0; i < 132; i += 2)
    {
        SERPENT(l_key)[i + 8] = rotr(i ^ SERPENT(l_key)[i] ^ t2, 21);

        t1 ^= SERPENT(l_key)[i + 2] ^ SERPENT(l_key)[i + 8];

        SERPENT(l_key)[i + 9] = rotr((i + 1) ^ SERPENT(l_key)[i + 1] ^ t1, 21);

        t2 ^= SERPENT(l_key)[i + 3] ^ SERPENT(l_key)[i + 9];
    }
...
void SERPENT(encrypt(const u1byte in_blk[16], u1byte out_blk[16]))
...
void SERPENT(decrypt(const u1byte in_blk[16], u1byte out_blk[16]))
{   u4byte  a,b,c,d,e,f,g,h;
    u4byte  t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13,t14,t15,t16;
