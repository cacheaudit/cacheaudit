  ...
  for domain in domains:
    ...
    for csize in csizes:
      ...
      for p in progs:
        ...
        for line_s in line_sizes:
	  c_acc[strategy][domain][csize][p].setdefault(line_s,{})
	  c_accd[strategy][domain][csize][p].setdefault(line_s,{})
	  c_tr[strategy][domain][csize][p].setdefault(line_s,{})
	  c_time[strategy][domain][csize][p].setdefault(line_s,{})
	  ...
	  for i in range(len(lines) - 10,len(lines)):
	    ...
	    if line.startswith("Number of valid cache configurations :"):
	      ...
	      c_acc[strategy][domain][csize][p][line_s] = acc
	    elif line.startswith("      that is"):
	      accd = float(line.split()[-2]) if domain != "plru" else -1.0
	      c_accd[strategy][domain][csize][p][line_s] = accd
	    elif line.startswith(" # traces:"):
	      c_tr[strategy][domain][csize][p][line_s] = log(int(line.split()[-1]),2)
	    elif line.startswith(" times ("):
	      c_time[strategy][domain][csize][p][line_s] = float(line.split()[-2])
	    #elif line.startswith("Analysis took"):
	      #c_exec_time[strategy][domain][csize][p] = int(line.split()[-2])

ofile_acc = open("acc-line.data","w")
ofile_accd = open("accd-line.data","w")
ofile_tr = open("tr-line.data","w")
ofile_time = open("time-line.data","w")
...
for strategy in strategies:
  ...
  for p in progs:
    ...
    for domain in domains:
      ...
      for line_s in line_sizes:
	for of in ofiles:
	  of.write(str(line_s) + "B\t")
	for csize in csizes:
	  ofile_acc.write("%.3f\t" % c_acc[strategy][domain][csize][p][line_s])
	  ofile_accd.write("%.3f\t" % c_accd[strategy][domain][csize][p][line_s])
	  ofile_tr.write("%.3f\t" % c_tr[strategy][domain][csize][p][line_s])
	  ofile_time.write("%.3f\t" % c_time[strategy][domain][csize][p][line_s])
	for of in ofiles:
	  of.write("\n")
