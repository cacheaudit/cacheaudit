...
int main(){
  ...
  unsigned long mylength=128;
  unsigned char mykey[16];
  unsigned char myinput[128];
  unsigned char myoutput[128];
  unsigned char iv[16];
...
while( length > 0 )
        ...
        aes_crypt_ecb(ctx, output,output);
	for( i = 0; i < 16; i++ )
            iv[i] = output[i];
