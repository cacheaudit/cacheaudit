...
let init n = Array.init n (fun _ -> Int64.zero)
...
let get_value_16 arr reg =
      ...
      Int64.logand x_reg 0xFFFFL
let set_value_16 arr reg value =
      let reg_num = reg16_to_int reg in
      let value_mask = Int64.logand value 0xFFFFL in
      let reg_masked = Int64.logand (Array.get arr reg_num) 0xFFFF0000L in
      Array.set arr reg_num (Int64.logor reg_masked value_mask)
...
let set_value_8 arr reg value =
      ...
      let hl_reg = Array.get arr (reg_num mod 4) in
      let value_mask = Int64.logand value 0xFFL in
      let value_shift = Int64.shift_left value_mask (fst hilo) in
...
let print_regs arr =
      ...
      Array.iteri (fun i x -> Format.printf "@[%s\t0x%Lx@]@." (int_r32_str i) x) arr; print_newline ()
